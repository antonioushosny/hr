        ...
        <div class="row">
            <div class="col-lg-5 col-md-5 col-sm-12">
                ...
                @endif
                    ...
                    <li class="breadcrumb-item "><a href="javascript:void(0);">{{__('admin.edit_task')}}</a></li>
        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card">
                        ...
                        <div class="header">
                            <h2><strong>{{trans('admin.'.$title)}}</strong> {{trans('admin.edit_task')}}  </h2>
                            
                        </div>
                        <div class="body row">
                            <div class="col-lg-10">
                                {!! Form::open(['route'=>['storetask'],'method'=>'post','name'=>"myform",'autocomplete'=>'off', 'id'=>'form_validation', 'enctype'=>'multipart/form-data' ])!!} 
                                     <div class="form-group form-float">
                                        <input type="hidden" value="{{$data->id}}" name="id" required>
                                    </div>
                                    <div class=" row">
                                        <div class="form-group form-float col-lg-6">
                                            <input type="text" value="{{ $data->title }}" class="form-control" placeholder="{{__('admin.title')}}" name="title" required>
                                            <label id="title-error" class="error" for="title" style="">  </label>
                                        </div>
                                        <div class="form-group form-float col-lg-6">
                                            <input type="date"  value="{{ $data->date }}" class="form-control" placeholder="{{__('admin.date')}}" name="date"  required>
                                            <label id="date-error" class="error" for="date" style=""></label>
                                        </div>
                                        <div class="form-group form-float col-lg-6">
                                            <input type="text"  value="{{ $data->time }}" class="form-control" placeholder="{{__('admin.time')}}" name="time" maxlength= 14 onkeypress="isNumber(event);" autocomplete="off" required>
                                            <label id="time-error" class="error" for="time" style=""></label>
                                        </div>
                                        <div class="form-group form-float col-lg-6">
                                            <input type="text"  value="{{ $data->project_name }}" class="form-control" placeholder="{{__('admin.project_name')}}" name="project_name" required>
                                            <label id="project_name-error" class="error" for="project_name" style="">  </label>
                                        </div>
                                        <div class= "form-group form-float col-lg-6"> 
                                            {!! Form::select('employee_id',$employees
                                                ,$data->employee_id,['class'=>'form-control show-tick' ,'placeholder' =>trans('admin.employee_id'),'required']) !!}
                                        ...
                                        <div class= "form-group form-float col-lg-6"> 
                                            {!! Form::select('status',['waiting'=>'Waiting','working on'=>'Working On','done'=>'Done','not completed'=>'Not Completed',]
                                                ,$data->status,['class'=>'form-control show-tick' ,'placeholder' =>trans('admin.status'),'required']) !!}
