...
<section class="content home">
        ...
        <div class="row">
            <div class="col-lg-5 col-md-5 col-sm-12">
                ...
                @endif
                    ...
                    <li class="breadcrumb-item"><a href="javascript:void(0);"><i class="zmdi zmdi-face"></i> {{__('admin.vacations')}}</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card">
                {!! Form::open(['route'=>['vacationsdeleteall'],'method'=>'post','autocomplete'=>'off', 'id'=>'vacationss_form' ])!!}
                                    ...
                                    @foreach ($vacations as $data)
                                     <tr class="item{{$data->id}}">
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('acceptvacation',$data->id)}}" class="btn btn-warning waves-effect waves-float waves-green btn-round " title="{{trans('admin.accept')}}">{{trans('admin.accept')}}</a> 

                                            <a href="{{ route('rejectvacation',$data->id)}}" class="btn btn-danger waves-effect waves-float waves-green btn-round " title="{{trans('admin.reject')}}">{{trans('admin.reject')}}</a> 
                                            @endif
        ...
        id = $(this).data('id') ;
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: 'GET',
                        url: "<?php echo url('/')?>/vacations/delete/" + id,
                        data: {
                        ...
                        success: function(data) {
                            ...
                            window.location.replace("{{route('vacations')}}");
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                if(choices.length >= 1){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ URL::route("vacationsdeleteall") }}',
                        data:  new FormData($("#vacationss_form")[0]),
                        processData: false,
                        contentType: false,
                        success: function(data) {
